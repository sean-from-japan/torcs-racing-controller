...
The historical files here were written during the project, by a training or benchmark
script or — in one case noted below — by hand from one of those scripts' output. The
final container record is explicitly a later re-measurement. Files were renamed to
describe their stage; the original names are given so anything quoted here can be
traced back.

`best_lap_2plus_s` is the metric throughout: the fastest lap from lap 2 onwards. Lap 1
starts from rest and did not count in the league. The historical setup was solo on the
grid, damage off, in a 10-lap Quick Race on one host. The container measurement uses a
different setup and records it in full. Both are explained in
...
| `stage5_nn_ars_container_2026-09-08.json` | generated by `container/record_result.py` | **107.082 s** | Later pinned-container measurement: two warm laps, weight SHA and environment recorded |
...
the original development machine and has not been reproduced exactly elsewhere. The
pinned-container run on 2026-09-08 reached 107.082 s, recorded in
`stage5_nn_ars_container_2026-09-08.json`; see `models/README.md` for what this does and
does not establish.
